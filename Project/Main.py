...
best_tr_loss_history = None
best_val_loss_history = None
best_ssim_history = None
best_kld_history = None
...
for combo in combinations:
    ...
    try:  # Utilizziamo try ed except per non fermare il ciclo in caso di errori
        val_loss, ssim_list, KLD_list, avg_tr_loss_list, avg_val_loss_list = trainer.train_evaluate_model(
            config, 
            img_size = img_size,
            epochs=15, 
            X_train=X_train, 
        ...
        num_totale_immagini = len(val_loader.dataset)
        batch_size_gen = config['batch_size']
        ...
        percorso_pesi_appena_salvati = os.path.join(cartella_attuale, "migliori_pesi_galassie.pth")
        model.load_state_dict(torch.load(percorso_pesi_appena_salvati, weights_only=True))

        for _ in range(0, num_totale_immagini, batch_size_gen):
            # Generiamo un batch di galassie sintetiche
            fake_uint8 = genera_per_fid(model, batch_size_gen, config['latent_dim'], device)
        ...
        final_sum = fid_generazione + 500*val_loss
        if final_sum < global_best_loss:
            global_best_loss = final_sum
            best_config = config

            # Salvataggio dati per plot finale
            best_tr_loss_history = avg_tr_loss_list
            best_val_loss_history = avg_val_loss_list
            best_ssim_history = ssim_list
            best_kld_history = KLD_list
...
# Plot
plot = dual_plot()
plot.plot_error(avg_tr_loss_list, avg_val_loss_list)
plot.plot_metric(KLD_list, ssim_list)
...
## 6. Confronto tra immagini random

visuals.visualizza_confronto(final_model, test_loader, device)


## 7. Metamorfosi delle galassie

visuals.visualizza_metamorfosi_classi(final_model, X_test, Y_test, device, classe_A=2, classe_B=9)