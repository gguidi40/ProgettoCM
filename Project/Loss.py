...
class LossCalculator:
    def __init__(self, device):
        self.device = device
        # Spostiamo la metrica sul device
        self.ssim_module = StructuralSimilarityIndexMeasure(data_range=1.0).to(self.device)

    def vae_loss_ssim(self, recon, x, mu, logvar, beta=0.01):
        ...
        tra le due loss
        '''
        ssim_val = self.ssim_module(recon, x)  # ssim_val calcola quanto sono uguali due immagini, maggiore è meglio
        loss_ssim = 1 - ssim_val               # loss_ssim calcola la differenza tra le immagini, minore è meglio
